...
取り込む手段は 2 つあります。手元で `yuki-tool pull` してコミットするか、
**Actions → yukicoder の変更を取り込む → Run workflow** を実行して取り込み PR を
作るかです。定期実行はしません。

取り込みを main に入れてからリポジトリ側の編集を進めてください。ずれたまま main に何か入ると、
...
`problem.toml` の `judgeType` を "スペシャル" にしてください ("リアクティブ" や数値でも
書けます)。"通常" のままだと保存はできても使われません (`push` が警告を出します)。同じコミットで両方を変えれば 1 回で反映されます。
