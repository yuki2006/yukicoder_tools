...
- 検証中は前回の結果がそのまま返るため、`push` は「状態が確定していて、かつ検証時刻が
  テストケースの更新時刻より後」になるまで待ちます。前回の `AC` を今回の結果と
  取り違えないためです。
...
- validator の非終端 (実行中・実行待ち) の判定は、`GET /v1/statuses` が返す
  `category == "judging"` を正とします。ステータスの語彙は増えることがあるためです
  (実際に `Pending` が増えました)。この API を持たないサーバでは、組み込みの一覧
  (`WJ` / `WJ_PURE` / `Judge` / `Pending`) にフォールバックします。
  なお judging 以外になっても「結果が確定した」わけではありません
  (リジャッジ等で後から変わることがあります)。
- validator の完了判定は `latestCheck > testCaseLatest` です (`>=` ではありません)。
  サーバのタイムスタンプは秒精度なので、同一秒に「テストケース更新 → 前回の検証完了の
  記録」が並ぶと `=` になり、前回の結果を今回の結果と誤判定するためです。
