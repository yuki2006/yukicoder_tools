...
- 問題文・解説・ジェネレータ・ジャッジコード・validator は、読み書きとも改行を LF に揃えます
...
- テストケース名に使える文字は、サーバの規則 (`GET /v1/testcase_name_rule`) に従います。
...
コンパイル状態が判定中 (`GET /v1/statuses` の judging 分類) の間は待ち、抜けたら結果を
確認します (途中の状態を失敗として扱わない)。ソースを空にすると削除になり、この場合は
コンパイルが走らないので待ちません。
...
失敗時は詳細も表示します: `CE` ならコンパイルメッセージ (長いと途中で切れます)、
それ以外なら通らなかったケース名とそのステータスです。
...
- validator の非終端 (実行中・実行待ち) の判定は、`GET /v1/statuses` が返す
  ...
  持たないためです。なお judging 以外になっても「結果が確定した」わけではありません
